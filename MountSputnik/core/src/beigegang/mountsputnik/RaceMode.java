...
//			{"Energy/e1.png","Energy/e2.png","Energy/e3.png","Energy/e4.png","Energy/e5.png","Energy/e6.png","Energy/e7.png","Energy/e8.png","Energy/e9.png","Energy/e10.png"};
    ...
    private PositionMovementController movementController1;
    private PositionMovementController movementController2;
    ...
    private InputController input1 = InputController.getInstance(0);
    private InputController input2 = InputController.getInstance(1);
    ...
    public void writeNextStepJsonForAnimation(float lx, float ly, float rx, float ry, Array<Integer> pressed){
        ...
        input1 = InputController.getInstance(0);
        String released = "[";
...
//			}
        ...
        character2 = new CharacterModel(partTextures, world, DEFAULT_WIDTH / 2, Math.max(DEFAULT_HEIGHT/2, checkpoints.get(lastReachedCheckpoint)), scale);
        ...
        oldMovementController = new MovementController(character1);
...
//			}
    ...
    private void addCharacterToGame(CharacterModel c) {
    ...
    private void makeHandholdsToGripAtStart(CharacterModel c) {
    ...
    public void update(float dt, int id) {
        ...
        if (id == 0){
            ...
            input = InputController.getInstance(1);
...
//                oldMovementController.findAndApplyForces(nextToPress.get(0),iny,inx);
    ...
    private boolean checkIfDied(CharacterModel c) {
        return c.parts.get(HEAD).getY() < 0;

    }

    private boolean checkIfReachedCheckpoint(CharacterModel c) {
    ...
    private void dealWithSlipperyAndCrumblyHandholds(CharacterModel c) {
        for(int e : EXTREMITIES){
    ...
    private void cameraWork() {
        float character1Pos =( character2.parts.get(CHEST).getBody().getPosition().y + character1.parts.get(CHEST).getBody().getPosition().y)/2*scale.y;
    ...
    private void ungripAllFrom(HandholdModel h,CharacterModel c){
        for(int e : EXTREMITIES){
            ExtremityModel extremity = (ExtremityModel) c.parts.get(e);
    ...
    private void boundBodyVelocities(CharacterModel c) {
...
// ************************************START OBSTACLES*********************************************** //
    ...
    private void spawnObstacles(){
        float cpos = Math.max(character1.parts.get(CHEST).getBody().getPosition().y,character2.parts.get(CHEST).getBody().getPosition().y);
...
// ************************************END OBSTACLES*********************************************** //
     ...
     * @author Jacob
     */
    private HandholdModel[] glowHandholds(CharacterModel c) {
...
//			snapIfPossible(FOOT_RIGHT, hs);
     ...
     * @author Jacob
     * */
    private void snapIfPossible(int limb, HandholdModel[] hs, CharacterModel c) {
//		for (GameObject obj : objects) {
//			if (obj.getType() == GameObject.ObjectType.HANDHOLD) {
//				HandholdModel h = (HandholdModel) obj;
        HandholdModel closest = null;
        ...
        for (HandholdModel h: hs) {
            if (h == null) continue;
            for (Vector2 snapPoint : h.snapPoints) {
                dist = distanceFrom(limb, snapPoint,c);
                if (dist <= HANDHOLD_SNAP_RADIUS){
                    closest = closestdist > dist ? h:closest;
                    closestdist = closestdist > dist ? dist:closestdist;
...
//		}
    ...
    private double distanceFrom(int limb, Vector2 snapPoint, CharacterModel c) {
    ...
    private boolean isGripping(int part,CharacterModel c) {

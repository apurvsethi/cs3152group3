...
/**	both are updated every timestep with horizontal and vertical input from player */
	...
	public void update(float dt) {
		...
		if (nextToPress.size > 0) {
			...
			applyForce(nextToPress.get(0),forceL,true,input.getHorizontalL(),input.getVerticalL());
			...
			if (nextToPress.size > 1 && (TWO_LIMB_MODE)) {
				for (int ext:EXTREMITIES){
				 ...
				 applyForce(nextToPress.get(1),forceR,true,input.getHorizontalR(),input.getVerticalR());
...
//						return a negative number because that way the force for the limb can be channeled easily.
	...
	private float[] applyPhysicallyCorrectForceRightForearm(float jointAngle, float f, float v, float h){
//		jointAngle *= -1;
//		might have to add above line in, depending on angle testing.
		...
		float forceyH = f;
		Vector2 vect;
		//99% of the time (always true now).
		if(((RevoluteJoint)character.joints.get(FOREARM_RIGHT-1)).getUpperLimit() == FOREARM_PULLING_UPPER_LIMIT){
			if (v>0){ //up direction
				if (armJointAngle<45f){
					vect = forearmTo90(false);
					forcexV = vect.x;
					forceyV = vect.y;
				}else{
				...
				if (armJointAngle>-45f){
					...
					forcexV = -vect.x;
					forceyV = -vect.y;
//					TODO probably necessary eventually.
//					changeLimitsIfNecessary(FOREARM_RIGHT,FOREARM_SWITCHING_EITHER_WAY,FOREARM_PUSHING_LOWER_LIMIT,FOREARM_PUSHING_UPPER_LIMIT);
					//rotate joints to about 1 degrees, then set forearm joint limits from -100 to 1 degrees or whatever limits are
				}
			}
		...
		//values
			...
			//TODO Eventually write code detailing the times when the arm gets to this angle.
		}
		return new float[]{forcexV,forceyV,forcexH,forceyH};

	}
//		h *= -1;
//		float[] vals = applyPhysicallyCorrectForceLeftForearm(jointAngle,f,v,h,false);
//		float forcexV = vals[0];
//		float forceyV = vals[1];
//		float forcexH = vals[2];
//		float forceyH = vals[3];
//	}
	private Vector2 forearmTo90(boolean left){
	...
	private float[] applyPhysicallyCorrectForceLeftForearm(float jointAngle,float f, float v, float h,boolean left){
		...
		Vector2 vect;
		//99% of the time
		if(((RevoluteJoint)character.joints.get(FOREARM_LEFT-1)).getUpperLimit() == FOREARM_PULLING_UPPER_LIMIT){
			if (v>0){ //up direction
				if (armJointAngle<45f){
...
//					changeLimitsIfNecessary(FOREARM_LEFT,FOREARM_SWITCHING_EITHER_WAY,FOREARM_PUSHING_LOWER_LIMIT,FOREARM_PUSHING_UPPER_LIMIT);
	...
	private void applyPhysicallyCorrectForceRightArm(float jointAngle,float f, float v, float h) {
//		System.out.println(jointAngle);
		//necessary to make it line up with angles of left arm.
		jointAngle *= -1;
		float[] vals = applyPhysicallyCorrectForceLeftArm(jointAngle,f,v,h);
		float forcexV = vals[0];
		float forceyV = vals[1];
		float forcexH = vals[2];
		float forceyH = vals[3];
	...
	private float[] applyPhysicallyCorrectForceLeftArm(float jointAngle,float f, float v, float h){
		...
		//TODO this doesn't pull the player up at all, this just manuevers the arm into the correct position...
		if (v > 0) {
			if (absoluteAngle > -180 && absoluteAngle <= -90) {
